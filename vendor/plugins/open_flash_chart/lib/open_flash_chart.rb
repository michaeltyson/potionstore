class OpenFlashChart
  ...
  def self.swf_object(width, height, url)
    ...
    output += "<param name=\"movie\" value=\"#{ActionController::Base.relative_url_root||''}/open-flash-chart.swf?width=#{width.to_s} &height=#{height.to_s} &data=#{url}\" /><param name=\"quality\" value=\"high\" /><param name=\"bgcolor\" value=\"#FFFFFF\" />"
    output += '<embed src="' + (ActionController::Base.relative_url_root||'') + '/open-flash-chart.swf?width=' + width.to_s + '&height=' + height.to_s + '&data=' + url.to_s + '" quality="high" bgcolor="#FFFFFF" width="' +  width.to_s + '" height="' + height.to_s + '" name="open-flash-chart" align="middle" allowScriptAccess="sameDomain" type="application/x-shockwave-flash" pluginspage="http://www.macromedia.com/go/getflashplayer" />'
    output += '</object>'
		return output
  end
end
