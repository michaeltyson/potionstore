...
#   when 'risk-information-notification'
  ...
  def paypal_wps
    ...
    # PayPal IPN handler (refer config/paypal_wps.yml)
    if request.method != :post
      logger.warn("Got non-POST request to PayPal IPN from #{request.remote_addr || request.remote_ip}")
    ...
    if !order
      ...
      if params[:custom] && params[:custom] != 'optional' && order.valid_licensee_name(params[:custom])
        order.licensee_name = params[:custom]
      else
      ...
      if !order.tinydecode params[:item_number]
        logger.warn("Unable to decode order from item_number parameter, #{item_number} for customer #{params[:payer_email]}")
        order.status = 'F'
        order.failure_reason = "Unable to decode order from item_number parameter, #{item_number}"
